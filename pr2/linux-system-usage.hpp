...
namespace get_system_usage_linux
{
    ...
    inline Network_stats get_network_stats(const std::string& interface)
    {
        Network_stats stats = {0, 0};
        std::string line;

        std::ifstream rx_file("/sys/class/net/" + interface + "/statistics/rx_bytes");
        std::getline(rx_file, line);
        std::istringstream rx_iss(line);
        rx_iss >> stats.rx_bytes;

        std::ifstream tx_file("/sys/class/net/" + interface + "/statistics/tx_bytes");
        std::getline(tx_file, line);
        std::istringstream tx_iss(line);
        tx_iss >> stats.tx_bytes;

        return stats;
    }

    inline int get_network_usage(const std::string& interface)
    {
        Network_stats initial_stats = get_network_stats(interface);
        auto start_time = std::chrono::steady_clock::now();

        std::this_thread::sleep_for(std::chrono::milliseconds(1000));

        Network_stats final_stats = get_network_stats(interface);
        auto end_time = std::chrono::steady_clock::now();

        long long rx_bytes_diff = final_stats.rx_bytes - initial_stats.rx_bytes;
        long long tx_bytes_diff = final_stats.tx_bytes - initial_stats.tx_bytes;

        double rx_speed_mbps = (rx_bytes_diff * 8.0) / 1024 / 1024;
        double tx_speed_mbps = (tx_bytes_diff * 8.0) / 1024 / 1024;
        ...
        return max_speed ? (rx_speed_mbps + tx_bytes_diff) / max_speed * 100 : 0;
    }
